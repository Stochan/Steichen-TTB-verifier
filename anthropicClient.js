// Calls our own Vercel serverless proxy (/api/verify) which forwards the
// request to Anthropic server-side. This avoids the browser CORS restriction
// that blocks direct calls to api.anthropic.com from a deployed origin.
...
/**
 * Sends a base64 image + prompt to the /api/verify proxy and returns a parsed result.
 * The proxy forwards the request to Anthropic server-side, avoiding CORS issues.
 *
 * @param {string} apiKey   - Anthropic API key (forwarded in header to the proxy)
